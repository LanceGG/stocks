...
# 股票列表爬虫（data.eastmoney.com/zjlx/list.html）
STOCK_CAPITAL_FLOW_LIST_ONLY = True  # True=仅 stock 表；False=含板块/成分股/当日快照
STOCK_CAPITAL_FLOW_PAGE_SIZE = 50  # 每页条数
STOCK_CAPITAL_FLOW_MAX_PAGES = 0  # 0=全部页，调试可设 2
STOCK_CAPITAL_FLOW_CONCURRENT_REQUESTS = 8  # LIST_ONLY 模式并发
STOCK_CAPITAL_FLOW_CONCURRENT_REQUESTS_PER_DOMAIN = 4
STOCK_CAPITAL_FLOW_DOWNLOAD_DELAY = 0.5
SECTOR_CONSTITUENT_MAX_SECTORS = 0  # 全量模式：0=抓取全部板块成分股
...
STOCK_QUOTE_QFQ_SOURCE = "eastmoney"  # eastmoney=一次拉全量前复权；tencent=分段慢
EASTMONEY_PUSH2HIS_HOSTS = [
    "push2his.eastmoney.com",
    "82.push2his.eastmoney.com",
]
STOCK_QUOTE_CONCURRENT_REQUESTS = 16  # eastmoney 可较高；tencent 请设 4
STOCK_QUOTE_CONCURRENT_REQUESTS_PER_DOMAIN = 2
STOCK_QUOTE_DOWNLOAD_DELAY = 0.3
STOCK_QUOTE_DOWNLOAD_TIMEOUT = 60  # 腾讯分段请求较慢，默认 30s 易超时
STOCK_QUOTE_KLINE_DATALEN = 4000
STOCK_QUOTE_BULK_BATCH_SIZE = 500
STOCK_QUOTE_HFQ_BAR_COUNT = 2000  # 腾讯复权 K 单次条数（上限约 2000，超过接口返回空）
STOCK_QUOTE_QFQ_BAR_COUNT = 2000  # 前复权条数，不得超过 2000
STOCK_QUOTE_QFQ_CHUNK_DAYS = 700  # 前复权按自然日分段，避免单次约 640 根 K 线上限
STOCK_QUOTE_QFQ_MAX_RETRIES = 5  # 腾讯 qfq 空响应/失败重试
STOCK_QUOTE_QFQ_CONCURRENT_REQUESTS = 2  # 仅爬 qfq 时的并发上限
STOCK_QUOTE_FETCH_SINA = True  # 新浪不复权日 K
STOCK_QUOTE_FETCH_QFQ = True  # 腾讯前复权 qfq（缠论可仅开此项）
STOCK_QUOTE_FETCH_HFQ = False  # 腾讯后复权 hfq
